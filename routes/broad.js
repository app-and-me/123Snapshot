...
router.get('/broad', async (req, res) => {
    try {
      ...
      const [photoTotalCount, postTotalCount] = await Promise.all([
        Photo.count({ where: { yn: 1 }}),
        Post.count({ where: { yn: 1 }}),
      ]);
      ...
      if (photos.length > 0 || posts.length > 0) {
        return res.status(200).json({ message: "페이징된 사진 및 글 목록 불러오기 성공", photos, posts, pagination: results });
      } else {
